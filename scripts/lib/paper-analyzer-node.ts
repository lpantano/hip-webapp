...
- keyFindings: Extract 2-3 key findings from the abstract`;
  ...
  try {
    ...
    console.error('Error analyzing stance with Claude:', error);
    return fallbackStanceAnalysis(claimTitle, claimDescription, paper);
  }
}
...
function fallbackStanceAnalysis(
  ...
  return {
    stance,
    confidence,
    reasoning: 'Automatic keyword-based analysis (API key not configured for better AI analysis)',
    relevanceScore,
...
export async function analyzePaper(
  ...
  console.log(`Analyzing paper: ${paper.title.substring(0, 60)}...`);
